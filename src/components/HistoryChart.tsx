...
export default function HistoryChart({ clock, data, decision, metric = 'moisture', onMetricChange }: { clock: SessionClock; data: PotCase; decision: Decision; metric?: Metric; onMetricChange?: (value: Metric) => void }) {
  ...
  return <section className="panel history-panel" aria-labelledby="history-title">
    <div className="panel-heading"><h2 id="history-title"><ChartNoAxesCombined />Riwayat sensor</h2><label className="metric-picker">Parameter<SelectField label="Parameter grafik" value={metric} onValueChange={value => onMetricChange?.(value as Metric)} options={Object.entries(METRICS).map(([value, item]) => ({ value, label: item.label }))} /></label></div>
    ...
    <div className="point-detail"><label>Pilih sampel<SelectField label="Pilih sampel grafik" value={selected.t_h} onValueChange={value => setSelectedTime(Number(value))} options={rows.map(r => ({ value: r.t_h, label: `${clockDate(clock, r.t_h)} ${clockTime(clock, r.t_h)}` }))} /></label><div aria-live="polite"><strong>{spec.label}: {selected[metric] === null ? 'Tidak diukur' : `${selected[metric]} ${spec.unit}`}</strong><span>{status}</span><span>Waktu {`${clockDate(clock, selected.t_h)} ${clockTime(clock, selected.t_h)}`} {utcLabel(clock, selected.t_h)} · umur {Math.round((data.now_h - selected.t_h) * 60)} menit</span></div></div>
