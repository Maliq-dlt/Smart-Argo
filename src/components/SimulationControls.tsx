...
import { Play, Pause, Sprout, Droplets, SkipForward, ChevronDown, ListChecks } from 'lucide-react'
    ...
    <div className="simulation-control-body">
      ...
      {!sim.planted ? <><p className="section-hint">Mulai dari bibit. Waktu berjalan otomatis; rawat tanaman mengikuti kondisi media.</p><button className="button primary plant-seed" onClick={onPlant}><Sprout />Tanam & mulai</button></> : sim.source !== 'manual' && <>
        {complete ? <p className="journey-complete" role="status">Perjalanan selesai. Bentuk akhir mengikuti perawatan dalam model. Tinjau jurnal atau mulai pot baru.</p> : <>
          <div className="playback-controls"><button className="button primary" onClick={onRun} disabled={simulationStopped(session)}>{running ? <Pause /> : <Play />}{running ? 'Jeda' : 'Lanjutkan waktu'}</button><select aria-label="Laju waktu sensor" value={sim.rate} onChange={e => onChange({ rate: Number(e.target.value) })}><option value="1">Lambat</option><option value="12">Normal</option><option value="48">Cepat</option></select></div>
          <p className="playback-caption">1 hari = {sampleIntervalMs(sim.rate) * 48 / 1000} detik · sampel tiap 30 menit simulasi</p>
        </>}
      </>}
      {pauseNote && <div className="clock-state-note" role="status"><p>{pauseNote}</p>{sim.source === 'manual' && <button className="text-button" onClick={() => onChange({ source: 'auto' })}>Gunakan waktu otomatis</button>}</div>}
      {sim.source !== 'manual' && <p className="continuous-note">{sim.pauseOnCare ? 'Jeda otomatis aktif · hanya pada perhatian baru' : 'Waktu berjalan terus hingga dijeda'}</p>}
      {recommendation}
      ...
      <details className="control-details"><summary>Pengaturan waktu & sampel<ChevronDown /></summary><div className="details-content">
      ...
      {journey && <details className="control-details"><summary>Jadwal pupuk pribadi<ChevronDown /></summary><div className="fertilizer-schedule">
