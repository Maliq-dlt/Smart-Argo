...
**Simulasi** (`#simulasi`) berisi statistik di kiri, ilustrasi pertumbuhan di tengah, serta waktu/perawatan di kanan pada desktop. Di ponsel, ilustrasi dan kontrol tampil lebih dahulu. **Dashboard** (`#statistik`) berisi grafik tiga parameter, perbandingan sesi, konsol, JSON, dan jejak aturan. Navigasi tidak mengganti sesi atau menghentikan waktu yang sedang berjalan.

Sidebar membuka **Dashboard**, **Simulasi**, **Riwayat sesi**, **Bandingkan sesi**, **Status perangkat**, dan **Terminal**. Tombol sesi aktif pada toolbar membuka pustaka yang sama. Di ponsel, gunakan **Buka navigasi**; pemilihan tujuan menutup menu, sedangkan Escape mengembalikan fokus ke tombol menu. Tautan terminal/perangkat/perbandingan membuka disclosure tujuan dan memindahkan fokus ke judulnya.
...
1. Buka **Atur tanaman & skenario** untuk memilih cabai atau bawang, lalu tekan **Tanam & mulai**. Waktu langsung berjalan. Pot baru dimulai pada HST 0 dengan timestamp ketika **Tanam & mulai** ditekan. **Pot baru** mempertahankan perjalanan sebelumnya di **Lanjutkan sesi**.
...
4. **Siram 150 mL** mencatat tindakan. Respons sintetis diterapkan sekali pada sampel berikutnya, tanpa mengubah riwayat sebelumnya. Kondisi media yang kurang mendukung memperlambat perkembangan visual dalam model latihan.
...
HST di **Ilustrasi fase tanaman** tetap terpisah dari jam sensor dalam mode eksperimen. **Acuan lokal** mengubah observasi media dan jadwal kunjungan. Mengganti media memulai sesi baru. Setiap skenario, impor, dan mode sebelumnya dapat dilanjutkan tanpa menghapus eksplorasi lain.
...
Menu **Sesi** memuat **Lanjutkan sesi**, **Impor**, dan **Ekspor sesi**. **Ekspor sesi** mencadangkan sesi aktif sebagai JSON versi 2. **Impor** menerima versi 2, ekspor versi 1, dan kontrak kasus Python; keputusan selalu dihitung ulang. Versi 2 memulihkan metadata, draf, dan log lengkap, dengan ID baru agar sesi asal tetap utuh. Batas impor 2 MB / 10.000 sampel. Grafik menampilkan 120 sampel terakhir; log dibatasi 500 entri. Semua sampel sampai batas 10.000 tetap tersimpan dan ikut ekspor.
...
| Netlify / Cloudflare Pages | Base/root: `simulasi-web` sesuai struktur repo; build: `npm run build`; publish/output relatif ke root aplikasi: `dist`; Node 24. |
| cPanel / hosting statis | Ekstrak isi paket build ke `public_html/` atau subfolder tujuan. |

Tidak perlu rewrite SPA karena aplikasi memakai dua tampilan dengan navigasi hash. Situs publik sebaiknya memakai HTTPS. Persiapan ini tidak memublikasikan situs atau membuat akun hosting. Untuk deploy berbasis Git, buat/pakai repositori milik pengguna terlebih dahulu; workspace saat ini belum memiliki Git.
